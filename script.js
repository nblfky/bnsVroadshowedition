...
try {
  treasure = JSON.parse(localStorage.getItem(TREASURE_KEY) || '[]');
} catch (_) { treasure = []; }
...
function saveTreasure() {
  try { localStorage.setItem(TREASURE_KEY, JSON.stringify(treasure)); } catch(_){}
}
...
// --- Treasure Hunt rendering ---
function renderTreasure() {
  if (!treasureGrid) return;
  // Ensure number of slots equals treasureSteps length
  while (treasure.length < treasureSteps.length) treasure.push({ image: null, caption: '' });
  ...
  treasureGrid.querySelectorAll('.hunt-item').forEach((el, idx) => {
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', () => {
        treasure[idx] = { image: null, caption: '' };
        saveTreasure();
        renderTreasure();
      });
    }
